...
export function legacyRagAdminTarget(pathname: string): string {
  ...
  // The source ledger is active again under system management, so old bookmarks
  // should preserve their destination instead of silently landing on the document list.
  if (suffix === "ledgers") return "/admin/rag/ledgers";
  ...
  if (suffix === "jobs" || suffix === "publish") return "/admin/rag";
