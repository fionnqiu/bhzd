...
| **ReAct 框架** | LLM 自主选工具会破坏"写操作必须经用户显式确认"的核心安全约束（PRD §3）；教学场景需确定性。写确认路径：诊断/掌握度/预设走确认门（pending_confirmations）；Agent 学习任务创建自 2026-08-21 起改为任务草稿链路——LLM 只生成任务卡内容，学生点卡片「同步到学习任务」按钮即显式确认后落库 |
